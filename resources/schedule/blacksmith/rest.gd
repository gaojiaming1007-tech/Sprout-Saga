...
func update_day_schedule(npc: NPC):
    if !npc.machine.blackboard.get_var("move"):
        if GameManager.game.game_resource.hour == hour:
            ...
            if GameManager.game.game_resource.hour < hour && GameManager.game.game_resource.hour >= end_hour:
...
func finish(npc: NPC):
    ...
    var rest: BlacksmithWork = ResourceManager.load_resource("res://resources/schedule/blacksmith/work.tres")
